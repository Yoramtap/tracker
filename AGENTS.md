...
- For replay completion cues, derive a single status mapping from event type (`prd_created -> planned`, `story_shipped -> shipped`) in `replay-timeline.tsx` and drive both badge text and CSS `data-status` hooks from that shared mapping.
