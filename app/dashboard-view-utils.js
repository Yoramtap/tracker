...
(function initDashboardViewUtils(globalObject) {
  ...
  const DATA_SOURCE_URLS = {
    snapshot: getSourcePath("data", "backlog-snapshot.json"),
    prActivity: getSourcePath("data", "pr-activity-snapshot.json"),
    managementFacility: getSourcePath("data", "management-facility-snapshot.json"),
    productCycle: getSourcePath("data", "product-cycle-snapshot.json"),
    productCycleShipments: getSourcePath("data", "product-cycle-shipments-snapshot.json"),
    contributors: getSourcePath("data", "contributors-snapshot.json"),
    prCycle: getSourcePath("data", "pr-cycle-snapshot.json")
  };
  ...
  function syncRadioAvailability(name, allowedValues) {
    const allowed = new Set(
      (Array.isArray(allowedValues) ? allowedValues : []).map((value) => String(value))
    );
  ...
  function getRequiredSourceKeys(mode, availableSourceKeys = [], sectionKey = "community") {
    return dashboardRuntimeContract.getRequiredSourceKeys(mode, availableSourceKeys, sectionKey);
  }
